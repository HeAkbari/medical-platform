...
function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg
      viewBox="0 0 20 20"
      aria-hidden="true"
      className={`h-4 w-4 ${filled ? 'text-amber-400' : 'text-faint-foreground'}`}
      fill="currentColor"
    >
...
export function DoctorRating({
  ...
  const content = (
    <>
      ...
      <span className="font-medium text-subtle-foreground">{roundedRating.toFixed(1)}</span>
      {!compact ? (
        <span className={onClick ? 'text-brand underline-offset-2 group-hover:underline' : 'text-faint-foreground'}>
          ({reviewCount} reviews)
        </span>
      ) : null}
    </>
